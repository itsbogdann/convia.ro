import { useState, useEffect, useRef } from 'react';
import { useWidgetStore } from '../../store';
import { PortalHeader } from './PortalHeader';
import { usePortalColors } from './usePortalColors';
import { t } from '../../utils/translations';

const SearchIcon = () => (
  <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
    <circle cx="11" cy="11" r="8" />
    <line x1="21" y1="21" x2="16.65" y2="16.65" />
  </svg>
);

const FileIcon = () => (
  <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
    <path d="M14 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V8z" />
    <polyline points="14 2 14 8 20 8" />
  </svg>
);

export const SearchView = () => {
  const {
    theme,
    searchQuery,
    searchResults,
    searchLoading,
    searchKnowledge,
    setSelectedSearchResult,
    portalView,
    selectedSearchResult,
  } = useWidgetStore();
  const colors = usePortalColors();

  const [localQuery, setLocalQuery] = useState(searchQuery);
  const debounceRef = useRef<ReturnType<typeof setTimeout> | null>(null);
  const inputRef = useRef<HTMLInputElement>(null);

  useEffect(() => {
    inputRef.current?.focus();
  }, []);

  useEffect(() => {
    if (debounceRef.current) clearTimeout(debounceRef.current);
    debounceRef.current = setTimeout(() => {
      if (localQuery.trim()) {
        searchKnowledge(localQuery);
      }
    }, 300);
    return () => {
      if (debounceRef.current) clearTimeout(debounceRef.current);
    };
  }, [localQuery]);

  // Article detail view
  if (portalView === 'search:article' && selectedSearchResult) {
    return (
      <div className="lr-flex lr-flex-col lr-flex-1 lr-overflow-hidden">
        <PortalHeader
          title={selectedSearchResult.sourceName}
          onBack={() => setSelectedSearchResult(null)}
        />
        <div
          className="lr-flex lr-flex-col lr-flex-1 lr-overflow-y-auto lr-px-5 lr-py-4 lr-animate-fade-slide-up"
          style={{ backgroundColor: colors.chatBg }}
        >
          <div className="lr-flex lr-items-center lr-gap-2 lr-mb-3">
            <span className="lr-text-[10px] lr-font-medium lr-px-2 lr-py-0.5 lr-rounded-full" style={{ backgroundColor: colors.subtleBg, color: colors.secondaryText }}>
              {selectedSearchResult.source}
            </span>
            <span className="lr-text-[10px]" style={{ color: colors.mutedText }}>
              {Math.round(selectedSearchResult.score * 100)}% match
            </span>
          </div>
          <div className="lr-text-sm lr-leading-relaxed lr-whitespace-pre-wrap" style={{ color: theme.messages.assistantTextColor }}>
            {selectedSearchResult.content}
          </div>
        </div>
      </div>
    );
  }

  return (
    <div className="lr-flex lr-flex-col lr-flex-1 lr-overflow-hidden">
      <PortalHeader title={t(theme.translations, 'searchTitle')} backTo="home" />
      <div
        className="lr-flex lr-flex-col lr-flex-1 lr-overflow-hidden"
        style={{ backgroundColor: colors.chatBg }}
      >
        {/* Search input */}
        <div className="lr-px-4 lr-py-3">
          <div
            className="lr-flex lr-items-center lr-gap-2 lr-px-3 lr-py-2.5 lr-rounded-xl"
            style={{
              backgroundColor: theme.input.backgroundColor,
              border: `1px solid ${theme.input.borderColor}`,
            }}
          >
            <span style={{ color: colors.mutedText }}><SearchIcon /></span>
            <input
              ref={inputRef}
              type="text"
              value={localQuery}
              onChange={(e) => setLocalQuery(e.target.value)}
              placeholder={theme.portal?.searchPlaceholder || t(theme.translations, 'searchPlaceholder')}
              className="lr-flex-1 lr-text-sm lr-bg-transparent"
              style={{ color: theme.input.textColor, outline: 'none' }}
            />
            {searchLoading && (
              <svg width="18" height="18" viewBox="0 0 48 48" fill="none" className="lr-animate-pulse lr-shrink-0">
                <path fillRule="evenodd" clipRule="evenodd" d="M8.60917 36.9866C5.3218 33.3783 3.32191 28.6146 3.32191 23.3914C3.32191 12.1271 12.6184 2.99568 24.0861 2.99568C35.5539 2.99568 44.8504 12.1271 44.8504 23.3914C44.8504 31.3561 40.2009 38.2516 33.4239 41.6092C29.2775 43.771 24.5472 44.9957 19.5258 44.9957C13.3731 44.9957 7.65739 43.157 2.91748 40.0088C2.91748 40.0088 6.09606 39.6621 8.60835 36.9874L8.60917 36.9866ZM33.8397 33.4855C39.2117 28.1137 39.2117 19.4042 33.8397 14.0322C31.3487 11.5414 28.1401 10.2063 24.8794 10.0256V10.0248C24.781 7.57835 26.0979 5.92616 26.1043 5.91811L26.1036 5.91824L26.1045 5.9171C22.3569 6.64104 18.7783 8.45409 15.8764 11.356C13.6509 13.5814 12.0663 16.205 11.1216 18.9972C11.2591 18.6329 11.4124 18.2733 11.5814 17.9193C11.3865 18.4167 11.2103 18.925 11.051 19.445C9.47775 24.2209 10.5882 29.6873 14.3864 33.4855C19.7584 38.8575 28.4679 38.8575 33.8397 33.4855Z" fill="#c0c5cd"/>
                <path fillRule="evenodd" clipRule="evenodd" d="M17.1299 22.345V24.0539V25.1922C17.1299 26.3283 18.0509 27.2494 19.1872 27.2494C20.3234 27.2494 21.2443 26.3283 21.2443 25.1922V24.0522V22.345C21.2443 21.2089 20.3234 20.2877 19.1872 20.2877C18.0509 20.2877 17.1299 21.2089 17.1299 22.345Z" fill="#c0c5cd"/>
                <path fillRule="evenodd" clipRule="evenodd" d="M27.313 22.345V24.0539V25.1922C27.313 26.3283 28.2341 27.2494 29.3702 27.2494C30.5064 27.2494 31.4275 26.3283 31.4275 25.1922V24.0522V22.345C31.4275 21.2089 30.5064 20.2877 29.3702 20.2877C28.2341 20.2877 27.313 21.2089 27.313 22.345Z" fill="#c0c5cd"/>
              </svg>
            )}
          </div>
        </div>

        {/* Results */}
        <div className="lr-flex-1 lr-overflow-y-auto lr-px-4">
          {!localQuery.trim() ? (
            <p className="lr-text-sm lr-text-center lr-py-8" style={{ color: colors.mutedText }}>
              Type to search our knowledge base
            </p>
          ) : searchResults.length === 0 && !searchLoading ? (
            <p className="lr-text-sm lr-text-center lr-py-8" style={{ color: colors.mutedText }}>
              No results found
            </p>
          ) : (
            <div className="lr-space-y-2 lr-pb-4 lr-stagger" key={localQuery}>
              {searchResults.map((result, i) => (
                <button
                  key={i}
                  onClick={() => setSelectedSearchResult(result)}
                  className="lr-w-full lr-text-left lr-p-3 lr-rounded-xl lr-transition-all lr-card-interactive"
                  style={{
                    backgroundColor: theme.input.backgroundColor,
                    border: `1px solid ${theme.input.borderColor}`,
                    boxShadow: colors.cardStyle,
                  }}
                >
                  <div className="lr-flex lr-items-center lr-gap-2 lr-mb-1">
                    <span style={{ color: theme.header.backgroundColor }}><FileIcon /></span>
                    <span className="lr-text-xs lr-font-medium lr-truncate" style={{ color: theme.header.backgroundColor }}>
                      {result.sourceName}
                    </span>
                    <span className="lr-text-[10px] lr-ml-auto" style={{ color: colors.mutedText }}>
                      {Math.round(result.score * 100)}%
                    </span>
                  </div>
                  <p className="lr-text-sm lr-line-clamp-2" style={{ color: theme.messages.assistantTextColor }}>
                    {result.content.slice(0, 150)}...
                  </p>
                </button>
              ))}
            </div>
          )}
        </div>
      </div>
    </div>
  );
};
